// ThermalBenchTests — test suite (runner-agnostic).
...
func runAllTests() -> (total: Int, passed: Int, failed: Int) {
    ...
    test("ArchiveLoadResult: missing file degrades to unavailable") {
        let uuid = "archmiss-\(UUID().uuidString)"
        let path = SampleArchive.samplesFile(for: uuid).path
        let result = SampleArchive.loadDetailed(dataDirectory: path)
        try assertFalse(result.fileExists)
        try assertTrue(result.isFileBacked)
        try assertEqual(result.effectiveRawStatus(stored: .complete, expectedSamples: 10), .unavailable)
    }

    test("ArchiveLoadResult: duplicate lines degrade to partial") {
        // 5 lines on disk but the summary claims 4 — extra/duplicated lines
        // mean the file does not match the recorded run.
        let uuid = "archdup-\(UUID().uuidString)"
        var samples: [TelemetrySample] = []
        for i in 0..<4 {
            var s = TelemetrySample()
            s.elapsedSeconds = Double(i)
        ...
        try SampleArchive.append(samples, uuid: uuid)
        var extra = TelemetrySample()
        extra.elapsedSeconds = 99
        extra.cpuTempHottest = 99
        try SampleArchive.append([extra], uuid: uuid)
        let path = SampleArchive.samplesFile(for: uuid).path
        let result = SampleArchive.loadDetailed(dataDirectory: path)
        try assertEqual(result.totalLines, 5)
        try assertEqual(result.effectiveRawStatus(stored: .complete, expectedSamples: 4), .partial)
        SampleArchive.deleteFiles(uuid: uuid)
    }
    ...
    // ── Power-source / low-power change detection ─────────────────────
    test("Cancelled record captures power source change") {
        var samples: [TelemetrySample] = []
        for i in 0..<5 {
            var s = TelemetrySample()
            s.elapsedSeconds = Double(i)
            s.cpuTempHottest = 50
            s.acConnected = i < 2  // AC → battery mid-run
            s.lowPowerMode = i >= 3  // low power engages later
            s.tempValid = true
            samples.append(s)
        }
        ...
        try assertTrue(run!.powerSourceChanged)
        try assertEqual(run!.lowPowerModeAtStart, false)
        try assertEqual(run!.lowPowerModeAtEnd, true)
        try assertTrue(run!.lowPowerModeChanged)
    }

    test("Compare: power source changed during run warns") {
        let a = RunRecord.make()
        let b = RunRecord.make()
        a.powerSourceAtStart = true
        a.powerSourceAtEnd = false
        a.powerSourceChanged = true
        let r = CompareAnalyzer.analyze(a: a, b: b)
        try assertTrue(r.warnings.contains { $0.id == "power_changed_a" })
        try assertEqual(r.level, .comparableWithWarnings)
    }

    test("Compare: low power mode changed during run warns") {
        let a = RunRecord.make()
        let b = RunRecord.make()
        b.lowPowerModeAtStart = false
        b.lowPowerModeAtEnd = true
        b.lowPowerModeChanged = true
        let r = CompareAnalyzer.analyze(a: a, b: b)
        try assertTrue(r.warnings.contains { $0.id == "powerMode_changed_b" })
        try assertEqual(r.level, .comparableWithWarnings)
    }

    test("Compare: no change warnings for uniform runs") {
        let a = RunRecord.make()
        let b = RunRecord.make()
        let r = CompareAnalyzer.analyze(a: a, b: b)
        try assertFalse(r.warnings.contains { $0.id.hasPrefix("power_changed") })
        try assertFalse(r.warnings.contains { $0.id.hasPrefix("powerMode_changed") })
    }

    // ── Compare: effective raw status drives the verdict ──────────────
    test("Compare: effective raw status feeds analyzer verdict") {
        let a = RunRecord.make()
        let b = RunRecord.make()
        // Stored status says complete, but the file on disk is partial.
        let r = CompareAnalyzer.analyze(a: a, b: b, effectiveRawA: .partial)
        try assertTrue(r.warnings.contains { $0.id == "raw_partial_a" })
        try assertEqual(r.level, .comparableWithWarnings)
        let f = r.fields.first { $0.id == "rawData" }
        try assertNotNil(f)
        try assertTrue(f!.valueA.contains("Partial"))
    }

    test("Compare: unavailable effective status stays comparable but flagged") {
        let a = RunRecord.make()
        let b = RunRecord.make()
        let r = CompareAnalyzer.analyze(a: a, b: b, effectiveRawB: .unavailable)
        try assertTrue(r.canCompare)
        try assertTrue(r.warnings.contains { $0.id == "raw_unavailable_b" })
    }

    // ── Report (tally returned; printing is the runner's job) ─────────────
