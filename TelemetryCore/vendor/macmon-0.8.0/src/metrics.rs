...
pub struct TempMetrics {
  /// Highest CPU temperature in Celsius.
  pub cpu_temp_max: f32,
  /// Highest GPU temperature in Celsius.
  pub gpu_temp_max: f32,
  /// Number of CPU thermal-zone sensors.
  pub cpu_sensor_count: u32,
  /// Number of GPU thermal-zone sensors.
  pub gpu_sensor_count: u32,
...
impl Sampler {
  ...
  fn get_temp_smc(&mut self) -> WithError<TempMetrics> {
    ...
    let cpu_temp_max = cpu_metrics.iter().cloned().fold(f32::MIN, f32::max);
    let gpu_temp_avg = zero_div(gpu_metrics.iter().sum::<f32>(), gpu_metrics.len() as f32);
    let gpu_temp_max = gpu_metrics.iter().cloned().fold(f32::MIN, f32::max);

    Ok(TempMetrics { cpu_temp_avg, cpu_temp_max, gpu_temp_avg, gpu_temp_max,
                     cpu_sensor_count: cpu_metrics.len() as u32,
                     gpu_sensor_count: gpu_metrics.len() as u32 })
  }
  ...
  fn get_temp_hid(&mut self) -> WithError<TempMetrics> {
    ...
    let cpu_temp_max = cpu_values.iter().cloned().fold(f32::MIN, f32::max);
    let cpu_temp_max = if cpu_values.is_empty() { f32::NAN } else { cpu_temp_max };
    let gpu_temp_avg = zero_div(gpu_values.iter().sum(), gpu_values.len() as f32);
    let gpu_temp_max = gpu_values.iter().cloned().fold(f32::MIN, f32::max);
    let gpu_temp_max = if gpu_values.is_empty() { f32::NAN } else { gpu_temp_max };

    Ok(TempMetrics { cpu_temp_avg, cpu_temp_max, gpu_temp_avg, gpu_temp_max,
                     cpu_sensor_count: cpu_values.len() as u32,
                     gpu_sensor_count: gpu_values.len() as u32 })
  }
